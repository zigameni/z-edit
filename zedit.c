// Copyright [2025] Zigameni

/*** includes ***/

// These 3 lines are because the compiler complains about getline()
#include <stddef.h>
...
#define ZEDIT_VERSION "0.0.1"
#define ZEDIT_TAB_STOP 8
#define ZEDIT_QUIT_TIMES 3
...
int editorRowCxToRx(erow *row, int cx) {
  ...
  for (j = 0; j < cx; j++) {
    if (row->chars[j] == '\t')
      rx += (ZEDIT_TAB_STOP - 1) - (rx % ZEDIT_TAB_STOP);
    rx++;
  }
  return rx;
}
...
void editorUpdateRow(erow *row) {
  ...
  for (j = 0; j < row->size; j++)
    if (row->chars[j] == '\t')
      tabs++;
  free(row->render);
  row->render = malloc(row->size + tabs * (ZEDIT_TAB_STOP - 1) + 1);
  int idx = 0;
  for (j = 0; j < row->size; j++) {
    if (row->chars[j] == '\t') {
      row->render[idx++] = ' ';
      while (idx % ZEDIT_TAB_STOP != 0)
        row->render[idx++] = ' ';
    } else {
    ...
    if (filerow >= E.numrows) {
      if (E.numrows == 0 && y == E.screenrows / 3) {
        ...
        int welcomelen = snprintf(welcome, sizeof(welcome),
                                  "Z editor -- version %s", ZEDIT_VERSION);
...
void editorProcessKeypress() {
  static int quit_times = ZEDIT_QUIT_TIMES;
  int c = editorReadKey();
  switch (c) {
  case '\r':
    editorInsertNewLine();
